...
export class TradeRequestComponent implements OnInit {
  ...
  if(!(this.convert_DT(this.startdate))){
    alert('Start Date is not provided !');
    return;
  }
  if(!(this.convert_DT(this.enddate))){
  ...
  if(!((this.convert_DT((this.curr_date))&&(this.convert_DT(this.startdate)))<(this.convert_DT(this.enddate)))){
