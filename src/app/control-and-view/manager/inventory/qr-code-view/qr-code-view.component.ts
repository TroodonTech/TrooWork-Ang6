  ...
  ngOnInit() {
    ...
    this.inventoryService.getRoomDetailsList(this.roomKey$,this.OrganizationID).subscribe((data) => {
      this.roomdetails = data[0];
        this.qrcode = ConectionSettings.AbsUrl+'/#/UserWorkRequest/'+this.roomdetails.FacilityKey+'/'+this.roomdetails.FloorKey+'/'+this.roomdetails.ZoneKey+'/'+this.OrganizationID+'/'+this.roomKey$+'';
      });
