...
@Component({
  ...
  templateUrl: './generate-qr-code-list.component.html',
  styleUrls: ['./generate-qr-code-list.component.scss']
})
export class GenerateQrCodeListComponent implements OnInit {
  ...
  GenerateQRCodeRoomList() {

    var QRCodeRoomList = [];
    var QRCodeRoomString;

    if (this.checkValue.length > 0) {
      for (var j = 0; j < this.checkValue.length; j++) {
        if (this.checkValue[j] === true)
        QRCodeRoomList.push(this.RoomKeyValues[j]);
      }
      QRCodeRoomString = QRCodeRoomList.join(',');
    }
    this.checkvaluetag="workorderrequest"
    this.router.navigate(['/ManagerDashBoard', { outlets: { ManagerOut: ['QrCodeViewList', QRCodeRoomString, this.checkvaluetag] } }]);
  }

  checkBoxValueForQRCode(index, QRCodeCheckValue, RoomKey) {
    this.checkValue[index] = QRCodeCheckValue;
    this.RoomKeyValues[index] = RoomKey;
  ...
  GenerateFeedbackQRCodeRoomList() {

    var QRCodeRoomList = [];
    var QRCodeRoomString;

    if (this.checkValue1.length > 0) {
      for (var j = 0; j < this.checkValue1.length; j++) {
        if (this.checkValue1[j] === true)
        QRCodeRoomList.push(this.RoomKeyValues1[j]);
      }
      QRCodeRoomString = QRCodeRoomList.join(',');
    }
    this.checkvaluetag="reviewrequest"
    this.router.navigate(['/ManagerDashBoard', { outlets: { ManagerOut: ['QrCodeViewList', QRCodeRoomString, this.checkvaluetag] } }]);
  }

  checkBoxValueForFeedbackQRCode(index, feedbackQRCodeCheckValue, RoomKey) {
    this.checkValue1[index] = feedbackQRCodeCheckValue;
    this.RoomKeyValues1[index] = RoomKey;
    for(var i=0;i<this.checkValue1.length;)
    {
        if(this.checkValue1[i]==true)
        {
          this.feedcheckflag=true;
          return;
        }
        else
        {
          if(i==(this.checkValue1.length-1))
          {
            this.checkValue1=[];
            this.feedcheckflag=false;
            return;
          }
          i++;
        }
